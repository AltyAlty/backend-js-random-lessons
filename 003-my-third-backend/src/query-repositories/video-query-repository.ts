/*Используем здесь CQRS, то есть у нас здесь запрос, а не команда. Поэтому игнорируем BLL и занимаемся преобразованием
типов, которые приходят от сервера, в типы, которые должны поступить в UI, здесь, то есть не используя сервисы.*/
